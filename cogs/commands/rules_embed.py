...
class RulesEmbed(commands.Cog):
    ...
    async def _rules(self, ctx: Context):
        try:
            emb = (discord.Embed(
                title = f'Вітаємо всіх в {ctx.guild.name}',
                description = 'Насамперед хочу сказати що ми дуже раді бачити нових людей в нашому колективі, людей які дійсно захоплюються ІТ сферою та хочуть вивчати щось нове',
